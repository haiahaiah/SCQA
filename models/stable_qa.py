...
def stable_forward(
    ...
    if type(model) in [StableDistilBertForSpanQA]:
        outputs = encoder(
            input_ids,
    ...
    if start_positions is not None and end_positions is not None:
        ...
        total_loss = qa_loss + aux_loss_weight * stability_loss
...
class StableBertForSpanQA(BertPreTrainedModel):
...
class StableRobertaForSpanQA(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
...
class StableDistilBertForSpanQA(DistilBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
