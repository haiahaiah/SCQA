...
from transformers.models.distilbert import DistilBertModel, DistilBertPreTrainedModel
...
class Similarity(nn.Module):
    ...
    def forward(self, x, y, temp=0.05):
...
def cl_forward(
    ...
    if type(model) in [CLDistilBertForSpanQA]:
        outputs = encoder(
            input_ids,
    ...
    if start_positions is not None and end_positions is not None:
        ...
        cos_sim = model.sim(pooled_1.unsqueeze(1), pooled_2.unsqueeze(0), temp)
        labels = torch.arange(cos_sim.size(0)).long().to(input_ids.device)

        loss_fct = nn.CrossEntropyLoss()
        cl_loss = loss_fct(cos_sim, labels)

        total_loss = qa_loss + aux_loss_weight * cl_loss
...
class CLBertForSpanQA(BertPreTrainedModel):
    ...
    def forward(
        ...
        #     qa_loss = (start_loss + end_loss) / 2
        #
        #     # YH: contrastive learning loss
        #     # compute bs * bs similarity matrix
        #     # and labels is range(bs), loss_func=CE
        #     cl_loss = 0.0
        #     pooler_output = pooler_output.view((batch_size, num_example, pooler_output.size(-1)))  # (bs, num_sent, hidden)
        #     pooled_1, pooled_2 = pooler_output[:, 0], pooler_output[:, 1]
        #     cos_sim = self.sim(pooled_1.unsqueeze(1), pooled_2.unsqueeze(0), temp)
        #     labels = torch.arange(cos_sim.size(0)).long().to(input_ids.device)
        #
        #     loss_fct = nn.CrossEntropyLoss()
        #     cl_loss = loss_fct(cos_sim, labels)
...
class CLRobertaForSpanQA(RobertaPreTrainedModel):
    # _keys_to_ignore_on_load_unexpected = [r"pooler"]
...
class CLDistilBertForSpanQA(DistilBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
